...
class SpampedeData {
	...
	/**
	 * the number of spam that's eaten
	 */
	public int spamCount = 0;
	
	/**
	 * the number of lives that is left
	 */
	...
	public BoardCell getNorthNeighbor(BoardCell cell) {
		return this.boardCells2D[cell.getRow() - 1][cell.getColumn()]; //moving up a row so its -1 but stay in same column
	}
	...
	public BoardCell getSouthNeighbor(BoardCell cell) {
		return this.boardCells2D[cell.getRow() + 1][cell.getColumn()]; //moving down a row so its +1 but stay in same column
	}
	...
	public BoardCell getEastNeighbor(BoardCell cell) {
		return this.boardCells2D[cell.getRow()][cell.getColumn() + 1]; //stay in same row, but move over a col so its +1
	}
	...
	public BoardCell getWestNeighbor(BoardCell cell) {
		return this.boardCells2D[cell.getRow()][cell.getColumn() - 1]; //star in same row but move back a col so its -1
	}
	...
	public BoardCell getNextCellInDir() {
		...
		else //(this.currentMode == SnakeMode.GOING_WEST){
			return this.getWestNeighbor();
	}
	//what about edge cases?? do you go left or right at the end of the board?
	...
	/* ----------------------------------------- */
    /* Helper Methods to modify the data for the game */
	/* ----------------------------------------- */
	public void headHelperEatsSpam(BoardCell nextCell) {
		//first, make the old-head part of the body
		this.getSnakeHead().becomeBody();
		//second, make the spam to snakecells
		this.snakeCells.addLast(nextCell);
		//third, make the spamLocationCell a headcell
		nextCell.becomeHead();
		this.spamCells.remove(nextCell);
		//update the spamCount if you have eaten a spam! :)
		this.spamCount++;
	}
	
	public void headHelperNoSpam(BoardCell nextCell) {
		//first, make the oldhead part of the body
		this.getSnakeHead().becomeBody();
		//second, make the spam to snakecells
		this.snakeCells.addLast(nextCell);
		//third, make the spamLocationCell a headcell
		nextCell.becomeHead();
		//make the tail a boardcell
...
// we used remove instead of push and pop for the stack commands	
	
	public void reverseSnakeHelp() {
	// Step 1: unlabel the head
		this.getSnakeHead().becomeBody();
	// Step 2: reverse the body parts	
		Stack<BoardCell> rsnake = new Stack<BoardCell>();  //create the stack rsnake 
		
		while(snakeCells.size() > 0) {
			BoardCell snekkCell = snakeCells.removeFirst(); //we used removeFirst on the LinkedList snakecells
			rsnake.push(snekkCell);  //take everything off (push) snake and add it to the stack
		}
		while(rsnake.size() > 0) {
			BoardCell stackCell = rsnake.pop(); //pop this off the snake
			snakeCells.addLast(stackCell); //take everything back from the stack rsnake and put it back on linkedList SnakeCells
		}

	// Step 3: relabel the head
		snakeCells.getLast().becomeHead();  //labeled as head but doesnt affect directions
    // Step 4: calculate the new direction after reversing!
		BoardCell sneck = this.getSnakeNeck();
		BoardCell shead = this.getSnakeHead();
		
		if(getEastNeighbor(sneck) == shead) {
			this.setDirectionEast();                            //set the direction of list to be this cardinal direction
		
		} else if(getSouthNeighbor(sneck) == shead) {           //used the snake neck as an indicator for the
			this.setDirectionSouth();                           //direction of the snake
			
		} else if(getWestNeighbor(sneck) == shead) {
			this.setDirectionWest();
		
		} else if(getNorthNeighbor(sneck) == shead) {
	...
	//a helper function that help gets the neighbors 
	public BoardCell[] getCellNeighbors(BoardCell centerCell) {
		BoardCell[] myArray = {getNorthNeighbor(centerCell),
				getSouthNeighbor(centerCell),
				getEastNeighbor(centerCell), 
				getWestNeighbor(centerCell)};
		
		return myArray; //neighbors
	}
